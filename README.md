...
See [frontend setup and architecture](frontend/README.md) and [backend configuration, database, API and tests](backend/README.md). Development image uploads work with local MongoDB storage; production image uploads require Cloudinary configuration. The built-in price advisor uses real comparable listings; the first listing needs a manually researched price. An external prediction service remains optional. Contact/newsletter submission and password recovery are explicitly unavailable until real services are implemented.
