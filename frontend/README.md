...
During local development, product/profile images are stored persistently in MongoDB when all Cloudinary credentials are empty. Configured deployments use Cloudinary. Image previews are local until uploaded. The AI advisor defaults to a built-in nearest-neighbours regression model using real comparable product listings. No extra server or API key is needed. With no comparable listings, it explains that an initial researched price is required; it never invents training data. Set AI_PRICE_PROVIDER=external on the backend to use a separately configured prediction service. Manual pricing remains available. Historical charts use recorded listing snapshots, with units and districts kept distinct. No market history is generated in the browser.
