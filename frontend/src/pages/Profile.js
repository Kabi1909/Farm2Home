...
export default function Profile() {
  ...
  return (
    <>
      <PageHeading
        eyebrow={farmer ? 'TELL THE STORY BEHIND YOUR HARVEST' : 'MAKE YOURSELF AT HOME'}
        title={farmer ? 'Your farm profile' : 'Your profile'}
        action={
          !editing && (
      ...
      >
        <div className="profile-avatar">
          ...
          <div>
            <h2>{form.name}</h2>
            <p>{farmer ? 'Local grower' : 'Fresh food enthusiast'}</p>
          </div>
        </div>
        {editing && (
          <ImageUploader
        ...
        <div className="form-row">
          ...
          {farmer
            ? field('farm', 'Farm name', 'text', true)
        ...
        {farmer ? (
          <>
            ...
            <div className="form-row">
              {field('size', 'Farm size (acres)', 'number')}
              {field('experience', 'Years of experience', 'number')}
              {field('crops', 'Main crops')}
              <Select
                disabled={!editing}
                label="Farming method"
                options={methods}
                value={form.method || 'Conventional'}
          ...
          field('preferences', 'Preferred products')
        )}
        {error && (
        ...
        <button
          type="button"
          className="text-link"
          onClick={() => setPassword({ current: '', next: '', confirm: '', error: '' })}
        >
          Change password
        </button>
      </form>
