...
export default function Shop() {
  ...
  return (
    <main className="container page">
      <Breadcrumbs items={[{ label: 'Shop fresh produce' }]} />
      <PageHeading
        eyebrow="YOUR LOCAL FARMERS’ MARKET, ONLINE"
        title="A basket full of goodness."
        description="Discover what’s fresh, find your favourites, and support a local grower."
      />
      ...
      <div className="shop-layout">
        <aside className={'filters ' + (drawer ? 'open' : '')}>
          <div className="between">
            <h3>
              <SlidersHorizontal size={17} /> Find your fresh
            </h3>
            <button
              onClick={() => {
                setParams({});
          ...
          <Select
            ...
            options={[{ value: '', label: 'All towns' }, ...(towns[params.get('district')] || [])]}
          />
          {[
          ...
          <div className="filter-note">
            <Leaf size={22} />
            <strong>Every basket makes a difference.</strong>
            <p>Thank you for supporting Sri Lanka’s farming community.</p>
          </div>
        </aside>
        <section className="shop-results">
          <div className="results-bar">
            <span>
              <strong>{filtered.length}</strong> fresh finds
            </span>
            <Select
              label="Sort by"
...
export function Categories() {
  ...
  return (
    <main className="container page">
      <PageHeading
        eyebrow="A LITTLE OF EVERYTHING GOOD"
        title="Shop by category"
        description="From everyday essentials to something deliciously different."
      />
      <div className="category-grid large">
        {categories.map((name) => {
          const matching = products.filter((p) => p.category === name && p.enabled && !p.draft);
          return (
            <CategoryCard
              key={name}
              name={name}
              image={matching[0]?.images[0]}
