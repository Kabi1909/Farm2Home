...
export default function FarmerCard({ farmer: f }) {
  ...
  return (
    <Link to={'/farmers/' + f.id} className="farmer-card">
      ...
      <div>
        ...
        <div className="between">
          ...
          <span>
            {products.filter((p) => p.farmerId === f.id && p.enabled && !p.draft).length} products
          </span>
        </div>
      </div>
      <ArrowUpRight className="farmer-arrow" size={20} />
    </Link>
  );
}
