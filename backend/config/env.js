...
const schema = z.object({
  ...
  MONGO_URI: z.string().min(1), JWT_SECRET: z.string().min(32),
  JWT_EXPIRES_IN: z.string().regex(/^\d+[smhd]$/).default('7d'),
  FRONTEND_URL: z.string().url().default('http://localhost:5173'),
  AI_SERVICE_URL: z.string().url().default('http://localhost:8000'),
  AI_SERVICE_TIMEOUT_MS: z.coerce.number().int().min(100).max(30000).default(5000),
...
export function loadConfig(values = process.env) {
  ...
  if (!['http:', 'https:'].includes(new URL(config.AI_SERVICE_URL).protocol)) throw new Error('AI service must use HTTP(S).');
  return config;
}
