...
export async function list(req, res) {
  ...
  const filter = {
    user: {
      $in: await User.find({ role: "farmer", status: "active" }).distinct(
        "_id",
      ),
    },
  };
  ...
  if (query.search)
    filter.$or = ["farmName", "description", "city", "district"].map((key) => ({
      [key]: new RegExp(escapeRegex(query.search), "i"),
    }));
  ...
  res.json({
    success: true,
    data: result.data.map(publicFarmer),
    pagination: result.pagination,
  });
}
export async function detail(req, res) {
  const profile = await FarmerProfile.findOne({ user: req.params.id }).populate(
    { path: "user", select: "name profileImage", match: { status: "active" } },
  );
...
export async function mine(req, res) {
  respond(
    res,
    await FarmerProfile.findOne({ user: req.user._id }).populate(
      "user",
