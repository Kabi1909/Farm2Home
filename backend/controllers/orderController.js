...
export async function create(req, res) {
  ...
  respond(res, await checkout(req.user._id, req.validated.body, key, req.app.locals.config.DELIVERY_CHARGE), 'Orders created.', 201);
}
export async function list(req, res) {
  const filter = { [req.user.role === 'farmer' ? 'farmer' : 'customer']: req.user._id };
  const { status } = req.validated.query;
  if (status) filter.status = status === 'active' ? { $nin: ['Completed','Cancelled'] } : status === 'completed' ? 'Completed' : status === 'cancelled' ? 'Cancelled' : status;
  res.json({ success: true, ...await listPage(Order, filter, req.validated.query) });
}
